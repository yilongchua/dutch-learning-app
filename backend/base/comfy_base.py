...
class ComfyUIService:
    def __init__(self):
        ...
        self.comfy_output_dir = str(self.comfy_dir / "output")
        self.workflow_folder = str(Path(__file__).parent.parent / "workflow")

    def _load_workflow(self, json_file: str) -> Optional[dict]:
        """Loads the ComfyUI API JSON workflow."""
        workflow_path = os.path.join(self.workflow_folder, json_file)
        if os.path.exists(workflow_path):
            with open(workflow_path, "r") as f:
                workflow = json.load(f)
            return workflow
        else:
            print(f"Warning: Workflow not found at {workflow_path}")
            return None
    ...
    def set_video_filename(self, workflow:dict, file_prefix:str) -> dict:
        workflow["50"]["inputs"]["filename_prefix"] = file_prefix
        return workflow

    def set_img_filename(self, workflow:dict, file_prefix:str) -> dict:
        workflow["60"]["inputs"]["filename_prefix"] = file_prefix
        return workflow

    def input_image_prompt(self, workflow:dict, image_prompt:str) -> dict:
        workflow['83:27']["inputs"]["text"] = image_prompt
        return workflow

    def set_positive_image_prompt(self, workflow:dict, image_prompt:str) -> dict:
        workflow['6']["inputs"]["text"] = image_prompt
        return workflow

    def set_negative_image_prompt(self, workflow:dict, image_prompt:str) -> dict:
